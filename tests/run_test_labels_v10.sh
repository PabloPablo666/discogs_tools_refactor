...
SRC="${1:-/Users/paoloolivieri/discogs_store/raw/labels/discogs_20251101_labels.xml.gz}"

OUT_BASE="$DATA_LAKE/_tmp_test"
OUT="$OUT_BASE/labels_v10"
...
python3 "$(dirname "$0")/../pipelines/parse_labels_v10.py" \
  --src "$SRC" \
  --out "$OUT" \
  --clean \
  --batch 50000
...
print("LABELS sample:")
print(con.execute(f"""
  select label_id, name, parent_label_id, urls_csv
  from read_parquet('{p}')
