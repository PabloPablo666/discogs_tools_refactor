...
# validate and build JSON array for Digdag for_each
tmp=""
count=0
for rid in $(echo "$RUN_IDS_RAW" | tr ',' ' '); do
  ...
  if ! echo "$rid" | grep -Eq '^[0-9]{8}_[0-9]{6}(__([0-9]{8}|[0-9]{4}_[0-9]{2}))?$'; then
    echo "ERROR: invalid run_id: $rid" >&2
    exit 2
  fi
  if [ -z "$tmp" ]; then
    tmp="\"$rid\""
...
RUN_IDS_JSON="[$tmp]"
echo "[REGISTER_MANY_RUNS] parsed json: $RUN_IDS_JSON" >&2

# export variable to next tasks
echo "run_ids_json=$RUN_IDS_JSON" >> "$DIGDAG_ENV"
